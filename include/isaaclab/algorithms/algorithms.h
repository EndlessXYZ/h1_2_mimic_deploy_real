...
public:
    OrtRunner(std::string model_path)
    {
        ...
        for (size_t i = 0; i < session->GetInputCount(); ++i) {
            ...
            // Replace dynamic dimensions (negative values like -1 for 'batch_size') with 1
            for (auto& dim : shape) {
                if (dim < 0) dim = 1;
            }
    ...
    std::vector<float> act(std::unordered_map<std::string, std::vector<float>> obs)
    {
        ...
        // Build mapping from input index to obs data vector
        // If input name matches an obs group, use it directly
        // If not, and there's only one input + one obs group, use the single group as fallback
        std::vector<std::vector<float>*> input_data_ptrs;
        for (const auto& name : input_names) {
            auto it = obs.find(name);
            if (it != obs.end()) {
                input_data_ptrs.push_back(&it->second);
            } else if (input_names.size() == 1 && obs.size() == 1) {
                input_data_ptrs.push_back(&obs.begin()->second);
            } else {
        ...
        {
            auto& input_data = *input_data_ptrs[i];
