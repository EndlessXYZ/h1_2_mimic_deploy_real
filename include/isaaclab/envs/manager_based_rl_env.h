...
public:
    ...
    :cfg(cfg), robot(std::move(robot_))
    {
        ...
        { // default joint velocities (typically zero)
            robot->data.default_joint_vel = Eigen::VectorXf::Zero(robot->data.joint_ids_map.size());
        }
    ...
    void step()
    {
        ...
        // Cache for dump access
        last_observation.clear();
        for (const auto& [key, vec] : obs) {
            last_observation.insert(last_observation.end(), vec.begin(), vec.end());
        }
    ...
    std::vector<float> last_observation;  // flattened obs from observation_manager->compute()
    std::vector<float> last_raw_action;   // raw ONNX output before processing
};
