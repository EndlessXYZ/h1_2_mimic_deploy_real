...
public:
    ...
    /// Recompute a named observation term and refill its history buffer.
    /// Used after reset() to correct terms whose source data changed
    /// (e.g. last_action after setting the initial action).
    void recompute_term_history(const std::string& term_name)
    {
        for(auto & group : group_obs_term_cfgs_)
        {
            for(auto & term : group.second)
            {
                ...
                {
                    auto obs = term.func(this->env, term.params);
                    term.reset(obs);
                    return;
                }
            }
